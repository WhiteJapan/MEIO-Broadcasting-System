...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navItems.forEach(navItem => {
        navItem.addEventListener('click', () => {
            ...
            // ラベルテキストが含まれる最後のspanを取得
            const navText = navItem.querySelector('span:last-child').innerText;
    ...
    function syncThemeUI(theme) {
        ...
        const activeCard = document.querySelector(`.theme-card[data-value="${theme}"]`);
        if (activeCard) activeCard.classList.add('active');
        
        // スマホ用トグルの同期
        if (autoToggle) {
            autoToggle.checked = (theme === 'auto');
        }
    }
    ...
    themeCards.forEach(card => {
        card.onclick = () => {
            ...
            saveSettings();
            applySettings();
            syncThemeUI(settings.theme);
        };
    });

    if (autoToggle) {
        autoToggle.onchange = () => {
            // トグルONならauto、OFFなら直前の状態かlightへ
            settings.theme = autoToggle.checked ? 'auto' : 'light';
            saveSettings();
            applySettings();
            syncThemeUI(settings.theme);
        };
    ...
    // スマホ幅: 下部タブで「構成 / 案内 / 停車駅 / 選択」を切り替え
    (function initMobileTabs() {
        const bar = document.querySelector('.mobile-tab-bar');
        if (!bar) return;
        const mq = window.matchMedia('(max-width: 1000px)');
        const buttons = bar.querySelectorAll('.mobile-tab-btn');
        const panels = document.querySelectorAll('.mobile-tab-panel');
        const indicator = bar.querySelector('.tab-active-indicator');

        function updateIndicator(activeBtn) {
            if (!indicator || !activeBtn) return;
            // padding分（6px）を考慮してoffsetLeftから位置を調整
        ...
        function setMobileTab(tabId) {
            ...
            panels.forEach((panel) => {
                const on = panel.dataset.mobileTab === tabId;
                panel.classList.toggle('is-active', on);
                panel.setAttribute('aria-hidden', on ? 'false' : 'true');
            });
            const mainStack = document.querySelector('.main-sections.mobile-main-stack');
            if (mainStack) {
                mainStack.classList.toggle('mobile-main-stack-visible', tabId === 'builder' || tabId === 'guide');
            }
            
            // 少し遅延させて幅と位置を正しく取得（表示の切り替え直後だと取得に失敗することがあるため）
            if (activeBtn) {
        ...
        function syncForViewport() {
            ...
            const active = document.querySelector('.mobile-tab-panel.is-active');
            const id = active && active.dataset.mobileTab ? active.dataset.mobileTab : 'builder';
            setMobileTab(id);
        }
        ...
        window.addEventListener('resize', () => {
            if (mq.matches) {
                const activeBtn = document.querySelector('.mobile-tab-btn.is-active');
    ...
    // Changelog Logic (ローカルファイル制限回避のため JS変数を使用)
    function loadChangelog() {
        const textEl = document.getElementById('update-info-text');
        if (!textEl) return;
        
        try {
            const text = (typeof APP_CHANGELOG !== 'undefined') ? APP_CHANGELOG : '更新情報が見つかりません。';
            if (typeof marked !== 'undefined') {
                textEl.innerHTML = marked.parse(text);
            } else {
                textEl.innerText = text; // フォールバック
            }
        } catch (err) {
            console.error('Failed to load changelog:', err);
            textEl.innerHTML = '<p>更新情報の読み込みに失敗しました。</p>';
        }
    }
    
    const reloadChangelogBtn = document.getElementById('reload-changelog-btn');
    if (reloadChangelogBtn) {
        reloadChangelogBtn.addEventListener('click', loadChangelog);
    }
    
    // settings modalが開かれたら読み込む（または最初から読み込む）
    loadChangelog();
});